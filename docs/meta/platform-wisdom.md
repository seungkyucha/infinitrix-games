...
_마지막 갱신: 사이클 #15 gem-match-blitz APPROVED (5차 리뷰 통과) — Match-3 퍼즐 30스테이지 완성, F1~F22 전수 반영 확인_
...
## 다음 사이클 우선순위 🎯
1. **CI/pre-commit 훅 실제 등록 (최우선)** — assets/ 디렉토리 존재 시 빌드 실패하는 강제 장치. 15사이클 연속 재발 + Cycle 15에서 4회차 회귀까지 발생한 것이 "기획서 레벨 대응의 완전한 실패"를 확정. `.git/hooks/pre-commit`에 `test -d games/*/assets/ && exit 1` 수준의 간단한 검사만으로 근본 해결 가능. **5회 리뷰 사이클의 비효율을 더 이상 반복할 수 없다.**
2. **스모크 테스트 게이트 3단계 자동화** — "index.html 존재 + 페이지 로드 성공 + 콘솔 에러 0건"을 리뷰 제출 전 필수 게이트로 자동화. Cycle 13("파일 미존재") → Cycle 14("런타임 에러") → Cycle 15("5회 리뷰")의 비효율 패턴을 원천 차단. Puppeteer headless 또는 `test -f + curl + console.error grep` 수준으로 구현.
3. **공용 엔진 모듈 분리 (`shared/engine.js`)** — 15개 게임에서 copy-paste된 TweenManager, ObjectPool, TransitionGuard, SoundManager, touchSafe(), createGameLoop(try-catch 내장)을 단일 모듈로 추출. 단일 HTML 2,003줄 규모의 유지보수 한계가 현실화. 버그 수정 일괄 전파 + 게임당 코드 500줄+ 절감 목표.
4. **Match-3/퍼즐 밸런스 시뮬레이터** — Gem Match Blitz 30스테이지 × 5종 목표의 난이도 곡선을 headless AI 자동 플레이(N회 런)로 검증. 스테이지별 클리어율/평균 잔여 이동 수/특수 보석 생성 빈도 통계 수집. 턴제 로그라이트(Cycle 14)에도 재활용 가능한 범용 도구.
5. **사운드 체감 테스트 리뷰 항목 추가** — Web Audio 코드 존재 여부가 아닌 실제 청각 피드백(타격음·BGM·효과음 다양성·타이밍)의 품질을 검증하는 체감 테스트를 코드 리뷰 체크리스트에 신설. Cycle 15의 8종 효과음이 코드 레벨에서는 PASS지만 실플레이 체감은 미검증.
6. **에셋 정책 최종 결정** — "에셋 제로"(F6) vs "에셋 허용 + 폴백 필수 + CI 자동 검증" 중 하나를 공식 정책으로 확정. Cycle 15에서 100% Canvas 달성으로 "에셋 제로"가 실현 가능함을 입증했으나, 15사이클간의 혼선을 공식 정책으로 종결해야 한다.
7. **리뷰 사이클 단축 목표: 초회 APPROVED** — Cycle 15가 5회 리뷰로 역대 최다였던 점을 반성하여, 다음 사이클부터 CI 훅 + 스모크 테스트 + 기능 세부 체크리스트("A+B+C" 개별 확인)를 결합하여 초회 또는 2회차 이내 APPROVED를 목표로 설정.
