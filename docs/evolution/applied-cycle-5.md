...
- 🟡 MEDIUM #2 **[art/METRIC_FIX]** stylePurity 비교에 토큰 단위 매치 추가 — canonical 을 하이픈/공백으로 split, 모든 토큰이 manifest 에 존재하면 MATCH → `agents/src/metrics.ts`
- 🟡 MEDIUM #3 **[qa/METRIC_FIX]** reviewRounds 를 review 파일 YAML front-matter `reviewRound: N` 에서 1차 우선 추출, 실패 시 기존 regex 폴백 → `agents/src/metrics.ts`
